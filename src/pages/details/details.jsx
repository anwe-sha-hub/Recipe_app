 ...
 console.log(recipeDetailsData, "recipeDetailsData");
  ...
  return (
    <div className=' container mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-10'>
      <div className='h-96 overflow-hidden rounded-cl group'>
    ...
    <div className='flex flex-col gap-3'>
      ...
      <div>
        <button onClick={()=>handleAddToFavorite(recipeDetailsData?.recipe)} className='p-3 px-8 rounded-lg text-sm uppercase font-medium tracking-wider mt-3 inline-block shadow-md bg-black text-white'>
          {
            favoritesList && favoritesList.length>0 && favoritesList.findIndex((item)=>item.id ===recipeDetailsData?.recipe?.id)!==-1 ?"Remove from favourites":"Add to favorites"}
      ...
      recipeDetailsData?.recipe?.ingredients.map(ingredient=><li>
        <span className='text-xl font-semibold text-red-600 '>{ingredient.quantity}{ingredient.unit} </span>
        <span className='text-xl font-semibold text-gray-500 '>{ingredient.description}</span>
      </li>
      )
    }
      </ul>
